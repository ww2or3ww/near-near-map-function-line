...
LINE_CHANNEL_ACCESS_TOKEN   = ""    if("LINE_CHANNEL_ACCESS_TOKEN" not in os.environ)   else os.environ["LINE_CHANNEL_ACCESS_TOKEN"]
LINE_CHANNEL_SECRET         = ""    if("LINE_CHANNEL_SECRET" not in os.environ)         else os.environ["LINE_CHANNEL_SECRET"]
    ...
    try:
        ...
        def message(line_event):
            ...
            if word in NEAR_NEAR_HELP or not user_info:
                ...
                LINE_BOT_API.reply_message(line_event.reply_token, make_text_message("こんにちは！にゃーにゃーマップです♪\n「にゃーにゃー」と呼びかけてにゃ～♪"))
...
def reply_by_nearnearmap_api(line_event, query_param, selected_type):
    ...
    if not result:
        ...
        for i in range(len(result_list)):
            messages.append(make_location_message(result_list[i]["title"], "{:.1f} km".format(result_list[i]["distance"]), result_list[i]["lat"], result_list[i]["lng"]))
...
def search_from_nearnearmap_api(query_param, type_word):
    url = "https://l8h2fp9jcf.execute-api.ap-northeast-1.amazonaws.com/work/near-near-map-search?{0}".format(query_param)
